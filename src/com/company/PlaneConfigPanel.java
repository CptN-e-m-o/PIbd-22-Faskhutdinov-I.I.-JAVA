...
public class PlaneConfigPanel extends JDialog {

    private Plane transport;
    private Color transportColor;
    private IAdding transportAdding;
    private DrawPicture drawPicture;

    public PlaneConfigPanel(Frame frame) {
        super(frame, true);
        setSize(900, 450);
        setLayout(null);
        ...
        JLabel planeLable = new JLabel("Самолёт");
        planeLable.setBorder(BorderFactory.createLineBorder(Color.BLACK, 1));
        planeLable.setBounds(10, 30, 190, 55);
        planeLable.setHorizontalAlignment(SwingConstants.CENTER);

        JLabel hydroplaneLable = new JLabel("Гидроплан");
        hydroplaneLable.setBorder(BorderFactory.createLineBorder(Color.BLACK, 1));
        hydroplaneLable.setBounds(10, 100, 190, 55);
        hydroplaneLable.setHorizontalAlignment(SwingConstants.CENTER);
        ...
        //JSpinner spinnerFloatCount = new JSpinner(new SpinnerNumberModel(4, 4, 6, 1));
        JSpinner spinnerFloatCount = new JSpinner(new SpinnerNumberModel(2, 2, 6, 2));
        ...
        MouseAdapter listenerTransportType = new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent e) {
                ...
                switch (label.getText()) {
                    case "Самолёт" -> transport = new Plane((int) spinnerMaxSpeed.getValue(), (int) spinnerWeight.getValue(), Color.WHITE);
                    case "Гидроплан" -> transport = new Hydroplane((int) spinnerMaxSpeed.getValue(), (int) spinnerWeight.getValue(), Color.WHITE, Color.WHITE,
        ...
        planeLable.addMouseListener(listenerTransportType);
        hydroplaneLable.addMouseListener(listenerTransportType);
        panelTypeOfTransport.add(planeLable);
        panelTypeOfTransport.add(hydroplaneLable);
        ...
        MouseListener listenerPanelDrawTransport = new MouseAdapter() {
            ...
            public void mouseEntered(MouseEvent e) {
                if (transport != null || (drawPicture.getPlane() instanceof Hydroplane && transportAdding != null)) {
            ...
            public void mouseReleased(MouseEvent e) {
                if (drawPicture.getPlane() != null) {
                    if (e.getX() + ((JComponent) e.getSource()).getX() >= labelMainColor.getX() &&
                        ...
                        drawPicture.getPlane().setMainColor(transportColor);
                        repaint();
                            ...
                            drawPicture.getPlane() instanceof Hydroplane) {
                        Hydroplane hydroplane = (Hydroplane) drawPicture.getPlane();
                        hydroplane.setDopColor(transportColor);
                        repaint();
                    }
                }
                setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
            ...
            public void mouseEntered(MouseEvent e) {
                ...
                switch (labelColor.getText()) {
                    ...
                    case "Дополнительный цвет" -> {
                        if (drawPicture.getPlane() instanceof Hydroplane) {
                            setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
                        }
                    }
                }
        ...
        JLabel labelCircle = new JLabel("Круглые поплавки");
        ...
        JLabel labelCross = new JLabel("Треугольные поплавки");
        ...
        JLabel labelRect = new JLabel("Квадратные поплавки");
        ...
        JLabel labelTrackCount = new JLabel("Количество поплавков");

        labelTrackCount.setBounds(430, 30, 120, 15);
        spinnerFloatCount.setBounds(430, 50, 80, 25);

        panelAdditions.add(labelTrackCount);
        panelAdditions.add(spinnerFloatCount);
        ...
        MouseAdapter listenerAdding = new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent e) {
                ...
                switch (labelAdditions.getText()) {
                    case "Круглые поплавки" -> transportAdding = new FloatCircle((int) spinnerFloatCount.getValue() /2 - 1);
                    case "Треугольные поплавки" -> transportAdding = new FloatTriangle((int) spinnerFloatCount.getValue()/2 - 1);
                    case "Квадратные поплавки" -> transportAdding = new FloatRectangle((int) spinnerFloatCount.getValue()/2 - 1);
                }
            }
            ...
            public void mouseReleased(MouseEvent e) {
                if (drawPicture.getPlane() != null) {
                    if (e.getX() + ((JComponent) e.getSource()).getX() + panelAdditions.getX() >= drawPicture.getX() &&
                            ...
                            drawPicture.getPlane() instanceof Hydroplane) {
                        Hydroplane hydroplane = (Hydroplane) drawPicture.getPlane();
                        hydroplane.setAdding(transportAdding);
                        repaint();
                    }
                }
                setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
